...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-o3p2.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://dall-e-o3p2.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <section className=" max-w-7xl mx-auto">
      <div>
        <h1 className="font-extrabold text-[#222328] text-[32px]">
          ...
          <p className="mt-2 text-[#666e75] text-[16px] max-w-[500px]">
            Create imaginative and visually stunning images and share
      ...
      <form className=" mt-16 max-w-3x1" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          ...
          <button
            type="submit"
            className=" mt-3 text-white bg-[#6469ff] fond-medium
